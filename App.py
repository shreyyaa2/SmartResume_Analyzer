    ...
    if choice == 'Normal User':
        ...
        if pdf_file is not None:
            ...
            if resume_data:
                ...
                benchmark_value = benchmarks.get(reco_field, None)
                if benchmark_value and reco_field != "Unknown":
                    st.metric(
                        label="Benchmark Score for " + reco_field,
                        value=f"{benchmark_value} / 100",
                        delta=f"{score - benchmark_value} points vs. avg"
                    )
                else:
                        st.info("No benchmark available for this skill category.")
                ...
                fig = go.Figure(go.Indicator(
                        ...
                        gauge = {
                            ...
                            'bar': {'color': "#00C49A"},
                            'steps' : [
                                {'range': [0, 50], 'color': "#fabc10"},
                                {'range': [50, 80], 'color': "#1ed760"},
                                {'range': [80, 100], 'color': "#d73b5c"}
                            ],
                        }
                    ))
                st.plotly_chart(fig)
        ...
        if st.button('Login'):
            if ad_user == 'Shreya' and ad_password == 'mlhub123':
                st.success("Welcome Shreya !!!!!!")
                # Display Data
